...
def set_panel_size(panel: figure, force_square: bool, height: int, width: int):
    left, right, bottom = compute_borders()
    top = int(TEXT_SIZE[:-2])
    ...
    if force_square:
        ...
        panel.min_border_bottom = left + right
...
def format_plot(plot, width: float, height: float, force_square: bool = False):
    ...
    def _format_single(fig):
        if isinstance(fig, figure):
            ...
            if fig.title:
                fig.title.text_font_size = TEXT_SIZE
                fig.title.text_font = TEXT_SIZE
            if fig.legend:
                fig.legend.label_text_font_size = TEXT_SIZE
