...
#########################
Perhaps the most simple example of a query is a `Vizier <https://vizier.cds.unistra.fr/>`_ query. For this example, we will retrieve data from GALEX. Queries of any kind use the :func:`~ATK.Tools.query` tool, which can be imported like this:
"""
...
# %%
# 2. The target's coordinates can be used directly by creating a :class:`~astropy.coordinates.SkyCoord`:
...
# Performing a Query
# ====================
# With a target set (in this case via the star's coordinates), we can now perform a GALEX Vizier query:

target = SkyCoord(12.2912, 5.3886, unit="deg", frame="icrs")
galex_query = query("vizier", targets=target, survey="galex")

# %%
# :func:`~ATK.Tools.query` returns a :class:`~ATK.Models.DataSet`, which contains information about the query along with any returned data. To see the structure of the returned :class:`DataSet <ATK.Models.DataSet>`, we can call its :meth:`~ATK.Models.DataSet.show` method:
...
# 
# - :attr:`~ATK.Models.DataSet.targets` lists the :class:`Targets <ATK.Models.Target>` that we entered for the search (coordinates, frame and epoch).
#
# - :attr:`~ATK.Models.DataSet.radius` gives the radius of the search.
#
# - :attr:`~ATK.Models.DataSet.exception` tells us whether any exceptions were encountered. If :attr:`~ATK.Models.DataSet.exception` is ``True``, then the query encountered something unexpected. The most common cause of this is that API we are utilising (in this case Vizier) is experiencing downtime.
#
# .. note::
# 
#    Passing ``show_types=True`` to :meth:`~ATK.Models.DataSet.show` forces printing of all attribute data types. 
#
# |
# 
# Any returned data is stored in the :attr:`~ATK.Models.DataSet.data` attribute. Unfortunately, targeting van Maanen's Star with its coordinates has returned an empty :class:`~ATK.Models.DataSet`. We can instead try to target it via its Gaia Source ID:
...
# %%
# While the attributes of a :class:`~ATK.Models.DataSet` describe the parameters of the query as it was requested, container attributes provide the details of the search as it was executed:
#
...
#
# - :attr:`~ATK.Models.Record.separation` gives the minimum separation between the position of the search and the returned Vizier catalogue rows.

# %%
# 
# |
#
# :class:`Records <ATK.Models.Record>` store data as a :class:`~pandas.DataFrame`, so we can extract a parameter from a catalogue (e.g. the Near-UV magnitude) as follows:
...
# Configuring a Query 
# ===================
# The arguments that can be passed to :func:`~ATK.Tools.query` depend on the ``kind`` of data that is being requested. Vizier queries do not have many optional arguments, but one example is the ``radius``, which defaulted to 3" here. The easiest way to override this is to set the ``radius`` explicity using :mod:`astropy.units`:
