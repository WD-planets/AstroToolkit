...
def skycoord_to_table(coord: SkyCoord, prefix: str = ""):
    ...
    has_pm = bool(coord.data.differentials)
    has_dist = coord.distance is not None

    table[f"{prefix}has_pm"] = has_pm
    table[f"{prefix}has_distance"] = has_dist
    table[f"{prefix}pm_ra_cosdec"] = coord.pm_ra_cosdec if has_pm else np.nan * (u.mas / u.yr)
    table[f"{prefix}pm_dec"] = coord.pm_dec if has_pm else np.nan * (u.mas / u.yr)
    table[f"{prefix}distance"] = coord.distance if has_dist else np.nan * u.pc
...
def read_skycoord(row: Row, prefix: str = ""):
    ...
    kwargs = dict(ra=ra, dec=dec, frame=frame, obstime=Time(epoch, format="fits"))
    ...
    if row[f"{prefix}has_pm"]:
        kwargs["pm_ra_cosdec"] = pmra
        kwargs["pm_dec"] = pmdec

    if row[f"{prefix}has_distance"]:
