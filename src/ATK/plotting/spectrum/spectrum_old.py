...
def get_overlay():
    ...
    wavelengths.append({"label": "Sodium", "wavelengths": [8183, 8195], "line_labels": [None, None]})
    wavelengths.append({"label": "Calcium", "wavelengths": [3934, 3967, 8498, 8542, 8662], "line_labels": [None, None, None, None, None]})
    wavelengths.append(
        {
            "label": "Calcium II",
...
def plot_spectrum(struct):
    ...
    plot = figure(
        ...
        y_axis_label=r"\[\text{flux / }10^{-16}\text{ erg}\text{cm}^{-2}\text{s}^{-1}\]" + "\u212b" + r"\[\:\:^{-1}\]",
    )
    ...
    for index, element in enumerate(wavelengths):
        annotation_count = 0
        annotation_group_size = len([x for x in element["line_labels"] if x is not None])
        for wavelength, annotation in zip(element["wavelengths"], element["line_labels"]):
            ...
            if annotation:
                annotation_height = max(y) + (annotation_count / annotation_group_size) * 0.3 * max(y)
                label = Label(x=wavelength, y=annotation_height, x_offset=2, text=annotation, text_font_size=text_size, text_font=text_font)

                plot.add_layout(label)
                annotation_count += 1

                line_renderer.js_on_change("visible", CustomJS(args=dict(ls=label), code="ls.visible = cb_obj.visible;"))
