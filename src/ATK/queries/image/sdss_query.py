...
def query(search_pos: SkyCoord, **kwargs) -> Image:
    band, size = kwargs["band"], kwargs["size"]

    # get image list
    imgs = SDSS.get_images(coordinates=search_pos, band=band, radius=size * u.arcsec)
    if not imgs:
        return RETURNS.NULL
    ...
    proj_hdu = reproject_hdu(hdu, search_pos, size)
