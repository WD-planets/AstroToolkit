...
def format_lc_data(lcs: list[Lightcurve] | Lightcurve):
    if isinstance(lcs, list):
        ...
        for lc in lcs:
            mjd.append(lc.mjd)
            brightness.append(lc._brightness)
            brightness_err.append(lc._brightness_err)
            band.append(np.full(len(lc.mjd), lc.band))
    else:
        mjd, brightness, brightness_err, band = (lcs.mjd, lcs._brightness, lcs._brightness_err, np.full(len(lcs.mjd), lcs.band))

    mjd = np.concatenate(mjd, axis=None)
...
def do_ls(lcs: list[Lightcurve] | Lightcurve, min: float, max: float, samples: int, return_model: bool = False):
    ...
    time_unit = mjd.unit if hasattr(mjd, "unit") else u.day
    freqs = np.linspace(min, max, samples) / time_unit
    ...
    if np.isnan(power).all():
        ...
        best_freq = freqs[np.nanargmax(power)].to(1 / u.day)
